...
public class MainController {
	...
	public String getHomepage(Model model){
		...
		model.addAttribute("ordersFinished", orders.stream().filter(order -> order.getFinishDate() != null).collect(Collectors.toList()));
		model.addAttribute("ordersInProgress", orders.stream().filter(order -> order.getFinishDate() == null).collect(Collectors.toList()));
		Map<String, List<Task>> tasksByImportance = tasks.stream().collect(Collectors.groupingBy(t -> String.valueOf(t.getImportance())));
		Map<String, Integer> tasksCountByImportance = new HashMap<>();
		tasksByImportance.keySet().forEach(tKey -> {
		...
		Map<String, List<Order>> ordersByDate = orders.stream().filter(o -> o.getFinishDate() != null).collect(Collectors.groupingBy(o -> new SimpleDateFormat("dd.MM").format(o.getFinishDate())));
		Map<String, Integer> ordersCountByDate = new HashMap<>();
		ordersByDate.keySet().forEach(tKey -> {
