...
public class OrderIntegrationTest {
  ...
  @Test
  public void givenSaveOrderMapping_whenNewOrderPOSTed_thenEntityAddedToRepository()
      throws Exception {
    ...
    this.mockMvc
        ...
        .andDo(print())
        .andExpect(status().is(302)); // 302 because of redirection, not needed
  ...
  @Test
  public void
      givenOrderAtRepositoryAndSaveOrderMapping_whenEditedOrderWithProdListPOSTed_thenEntitiesChangedAtRepositories()
          throws Exception {
    // given
    ...
    this.mockMvc
        .perform(
            post("/saveOrder")
                ...
                .param(
                    "productsList[0].id",
                    String.valueOf(order.getProductsList().stream().findFirst().get().getId()))
    ...
    assertEquals(
        new SimpleDateFormat("yyyy-MM-dd").parse("2002-02-10"),
        orderFound.getCreationDate()); // order.getDate()
    assertEquals(
        "1234567890", orderFound.getContractor().getNip()); // order.getContractor().getNip());
  ...
  @Test
  public void givenOrderWithOrderProducts_whenOrderDeleted_thenOrderProductsDeleted()
